...
class StageTests: XCTestCase {
    ...
    func testDelegateRandomLocations() {
        ...
        XCTAssertEqual(stage.randomLocations(1),[StageLocation(x: 1, y: 1)], "Gives a random free location")
    }
    
    func testCanAddApples() {
        stage = Stage.sharedStage
        stage.configurator = level1Config
        XCTAssertNil(stage.elements[Apple.className()], "Stage should not have nay apples")

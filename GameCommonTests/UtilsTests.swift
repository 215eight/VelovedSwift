...
class UtilsTests: XCTestCase {
    ...
    func testIntersectsStageElementArray() {
        ...
        var playerLocations = [StageLocation(x: 4, y: 8), StageLocation(x:4, y:9)]
        let firstPlayer = Player(locations: playerLocations, direction: .Up)
        
        playerLocations = [StageLocation(x: 30, y: 30), StageLocation(x: 29, y: 30)]
        let secondPlayer = Player(locations: playerLocations, direction: .Right)

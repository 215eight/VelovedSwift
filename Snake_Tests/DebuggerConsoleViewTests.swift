...
class DebuggerConsoleViewTests: XCTestCase {
    ...
    func testUpdateElementRefresh() {
        ...
        var apple = Apple(locations: [StageLocation.zeroLocation()], value: 10)
        var view = DebuggerConsoleView(cols: 3, rows: 1)
        view.updateElment(apple)

        XCTAssertEqual(view.description, "\(apple.locationDesc)  ", "Apple is drawn at the beginning of the line")
        XCTAssertEqual(view.elements.count, 1, "It contains the apple")

        apple.locations = [StageLocation(x: 1, y: 0)]
        view.updateElment(apple)

        XCTAssertEqual(view.description, " \(apple.locationDesc) ", "Apple was erased form the first position and drawn in the second position")
        XCTAssertEqual(view.elements.count, 1, "Still contains the apple")

        apple.locations = [StageLocation(x: 2, y: 0)]
        view.updateElment(apple)

        XCTAssertEqual(view.description, "  \(apple.locationDesc)", "Apple was erased form the second position and drawn in the third position")
        XCTAssertEqual(view.elements.count, 1, "Still contains the apple")
    }
    ...
    func testUpdateElement() {
        ...
        XCTAssertEqual(view.description, "\(apple.locationDesc) \n  ", "First character of first row should be a red apple")
    }
}
